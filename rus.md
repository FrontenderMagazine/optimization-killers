...
относится к Node.js, Opera и Chromium) оптимизировать ваши функции.
...
точно так же, как и машинный код. Это означает, что код работает быстро? Нет. Само по себе
компилирование не ускоряет работу код. Оно позволяет избежать накладных расходов,
вызванных работой интерпретатора, но код сам по себе будет ещё медленным, пока 
дело не дойдёт до оптимизации.
...
##### 5.2.1. Объекты в режиме «хэш-таблицы» («нормализованные объекты» или 
режим «словаря» — объекты, которые имеют хэш-таблицу в своём основании) не 
являются простыми перечисляемыми.
...
Возможность для свойства быть индексом массива описана в 
[спецификации ecmascript](http://www.ecma-international.org/ecma-262/5.1/#sec-15.4)
...
цикл, но также превращает содержащую такой `for-in` цикл в неоптимизируемую функцию.
