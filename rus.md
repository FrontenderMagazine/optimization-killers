...
Если это единственная причина использования `arguments`, то часто можно заменить на
...
- Никогда не используйте `arguments` напрямую без `.length` или `[i]` (ТОЛЬКО `x.apply(y, arguments)` МОЖНО использовать, ничего из оставшегося нельзя, например нельзя использовать `.slice`. `Function#apply` особенный случай).
...
И так если вы будете использовать в `for-in` цикле объект не являющийся простым
перечисляемым, это скажется на всей скажется на все содержащей его функции.
